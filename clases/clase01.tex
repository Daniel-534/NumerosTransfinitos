...
\begin{itemize}
  ...
  \item $s(0)$, $s(1)$, $s(s(0))$ son términos. Dado que $0$ y $1$ son constantes, $s(0)$ representa el sucesor de $0$ (es decir, $1$), $s(1)$ representa el sucesor de $1$ (es decir, $2$), y $s(s(0)) = s(1)$ también representa $2$.
...
\begin{enumerate}[leftmargin=2em, label=(\alph*)]
  ...
  \item \textbf{Interpretación en $\mathbb{Z}$:} Se puede interpretar $\Lang_{\mathrm{ar}}$ en $\mathbb{Z}$ con las operaciones usuales. En $\mathbb{Z}$, a diferencia de $\mathbb{N}$, la fórmula $\forall x\, \exists y\, (x = y + y)$ es falsa ($1$ no es el doble de ningún entero), mientras que $\exists x\, \exists y\, (x \neq y)$ sigue siendo verdadera. Esto ilustra cómo el valor de verdad de una fórmula depende de la estructura elegida.
\end{enumerate}
\end{ejemplo}
...
\textbf{Lectura:} $\wedge$I dice que si tenemos demostradas $\varphi$ y $\psi$ por separado, podemos concluir su conjunción. $\wedge$E$_1$ (resp. $\wedge$E$_2$) dice que de una conjunción podemos extraer el primer (resp. segundo) componente.
...
$(\Leftarrow)$ Supongamos que $\Gamma$ no tiene modelo. Por el Teorema de Completitud, $\Gamma$ es inconsistente: $\Gamma \vdash \bot$. Toda derivación en NK es una estructura finita y solo usa un número finito de hipótesis; por tanto, existe un subconjunto finito $\Delta \subseteq \Gamma$ con $\Delta \vdash \bot$. Por el Teorema de Corrección, $\Delta$ es insatisfacible: $\Delta$ no tiene modelo. Luego no todo subconjunto finito de $\Gamma$ tiene modelo.
\end{proof}
...
Todo subconjunto finito de $\Gamma_G$ involucra un número finito de vértices, por lo que corresponde a un subgrafo finito, que por hipótesis es $k$-colorable. Por Compacidad, $\Gamma_G$ tiene un modelo, que proporciona una $k$-coloración de $G$.
\end{proof}
