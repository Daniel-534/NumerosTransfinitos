...
\subsection{La ley de absorción: todo colapsa al máximo}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item $\aleph_0^{\aleph_0} \leq 2^{\aleph_0}$: cada función $f \colon \omega \to \omega$ puede codificarse biyectivamente mediante la biyección $p \colon \omega \times \omega \to \omega$ de Cantor. Concretamente, codificamos $f$ como la función característica de su gráfica: definimos $\hat{f} \colon \omega \to \{0,1\}$ no directamente, sino que usamos la biyección $q \colon \omega \to \omega \times \omega$ (inversa de $p$) para construir una función $F \colon \omega \to 2$ por $F(p(n,m)) = 1 \iff m = f(n)$. La asignación $f \mapsto F$ es inyectiva, luego $|{}^\omega\omega| \leq |{}^\omega 2| = 2^{\aleph_0}$.
\end{itemize}
...
\textbf{Parte 1 (inyección).} Para cada $i \in I$ fijamos una inyección $\phi_i \colon \kappa_i \hookrightarrow \lambda_i$ (existe porque $\kappa_i < \lambda_i$). Definimos $\Phi \colon \bigsqcup_{i \in I} \kappa_i \to \prod_{i \in I} \lambda_i$ por:
\[
  \Phi(\alpha, j)(i) = \begin{cases} \phi_j(\alpha) & \text{si } i = j, \\ 0_i & \text{si } i \neq j, \end{cases}
\]
donde $0_i = \min \lambda_i$ (el ordinal mínimo de $\lambda_i$, que existe por ser $\lambda_i > 0$). Verificamos la inyectividad: suponga $\Phi(\alpha, j) = \Phi(\beta, k)$. Evaluando en $i = j$: $\Phi(\alpha,j)(j) = \phi_j(\alpha)$ y $\Phi(\beta,k)(j) = \phi_k(\beta)$ si $k = j$, o $\Phi(\beta,k)(j) = 0_j$ si $k \neq j$. Evaluando en $i = k$: $\Phi(\alpha,j)(k) = 0_k$ si $j \neq k$, y $\Phi(\beta,k)(k) = \phi_k(\beta)$. Si $j \neq k$, de la igualdad en la coordenada $k$ se deduce $0_k = \phi_k(\beta)$; en particular $\phi_k(\beta) = 0_k$. Pero de la igualdad en la coordenada $j$ se deduce $\phi_j(\alpha) = 0_j$. Ahora: para cada $i \neq j$, $\Phi(\alpha,j)(i) = 0_i$, y para cada $i \neq k$, $\Phi(\beta,k)(i) = 0_i$; en la coordenada $j$ (con $j \neq k$), $\Phi(\beta,k)(j) = 0_j$, mientras que $\Phi(\alpha,j)(j) = \phi_j(\alpha) = 0_j$. La única posibilidad de conflicto surge si $0_j \in \mathrm{Im}(\phi_j)$ para algún $j$. Para evitarlo, sin pérdida de generalidad podemos tomar $0_i = \min(\lambda_i \setminus \mathrm{Im}(\phi_i))$ en lugar de $\min \lambda_i$ (este mínimo existe pues $|\mathrm{Im}(\phi_i)| \leq \kappa_i < \lambda_i$). Con esta elección, $\phi_j(\alpha) \neq 0_j$ para todo $\alpha \in \kappa_j$, luego de $\Phi(\alpha,j)(j) = \Phi(\beta,k)(j) = 0_j$ se concluye $j \neq k$ es imposible. Por tanto $j = k$, y entonces $\phi_j(\alpha) = \phi_j(\beta)$ (de la igualdad en la coordenada $j$), lo que implica $\alpha = \beta$ por ser $\phi_j$ inyectiva.
